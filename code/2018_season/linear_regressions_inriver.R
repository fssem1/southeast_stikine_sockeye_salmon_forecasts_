...
#data----
read_excel('data/2018_season/linear_regressions_data.xlsx', sheet="inriver") -> data

#data clean----
n <- 1 #small number to offset 0
names(data) <- c('year','statweek','weight','run', 'tah','tuya','cpuetah', 'catchtah',
                 'cpuetuya','catchtuya','cpue','catch')
...
log_data %>% 
  filter(!(year %in% c(1979, 1980, 1981, 1982, 1983, 1984, 1985, 1986, 1987, 1988, 1989, 1990, 1991, 1992, 1993, 1994)))  -> log_tuya
...
log_tuya %>% 
  do(tuya_catch = lm(tuya_log ~ catchtuya_log + statweek, data = .),
     tuya_cpue = lm(tuya_log ~ cpuetuya_log + statweek, data = .)) -> tuya_inriver

tuya_inriver %>% 
  tidy(tuya_catch) %>% 
  write_csv("output/inriver/lm_tuya_catch.csv")

tuya_inriver %>% 
  tidy(tuya_cpue) %>% 
  write_csv("output/inriver/lm_tuya_cpue.csv")

tuya_inriver %>% 
  augment(tuya_catch) %>% 
  filter(statweek %in% c('26', '27', '28', '29', '30', '31')) %>% 
  mutate(catch = exp(catchtuya_log), tuya = exp(tuya_log), fit = exp(.fitted)) %>% 
  ggplot(aes(catch, tuya)) + geom_point() + geom_line(aes(catch, fit)) + facet_wrap(~statweek,  labeller = label_both, dir = 'v', nrow = 3) +
  xlab("Cumulative Catch Tuya (inriver)") + ylab("Inriver Run (Tuya)") +
  scale_y_continuous(label=comma) + scale_x_continuous(label=comma) -> tah_catch
ggsave(file="figures/inriver/tuya_catch.png", plot=tuya_catch, width=10, height=8)

tuya_inriver %>% 
  augment(tuya_cpue) %>% 
  filter(statweek %in% c('26', '27', '28', '29', '30', '31')) %>% 
  mutate(cpue = exp(cpuetuya_log), tuya = exp(tuya_log), fit = exp(.fitted)) %>% 
  ggplot(aes(cpue, tuya)) + geom_point() + geom_line(aes(cpue, fit)) + facet_wrap(~statweek,  labeller = label_both, dir = 'v', nrow = 3) +
  xlab("Cumulative CPUE Tuya (inriver)") + ylab("Inriver Run (Tuya)") +
  scale_y_continuous(label=comma) + scale_x_continuous(label=comma) -> tah_cpue
ggsave(file="figures/inriver/tuya_cpue.png", plot=tuya_cpue, width=10, height=8)
